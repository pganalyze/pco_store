...
async fn timestamp() {
    ...
    let filter = Filter { database_id: vec![database_id], collected_at: Some(start..=end), ..Filter::default() };
    for group in CompressedQueryStats::delete(db, filter.clone(), fields).await.unwrap() {
    ...
    let group = CompressedQueryStats::load(db, filter.clone(), fields).await.unwrap().remove(0);
