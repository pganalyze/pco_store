...
async fn timestamp() {
    ...
    let start: chrono::DateTime<Utc> =
        DateTime::<Utc>::from(chrono::Utc::now() - Duration::seconds(3600)).duration_trunc(chrono::Duration::hours(1)).unwrap().into();
    ...
    // Write
    let collected_at = end - Duration::seconds(120);
    ...
    let collected_at = end - Duration::seconds(60);
    let stats = vec![QueryStat { database_id, collected_at, fingerprint: 1, calls: 1, total_time: 1.0 }];
    CompressedQueryStats::store(db, stats).await.unwrap();
    ...
    assert_eq!(group.start_at, end - Duration::seconds(120));
    assert_eq!(group.end_at, end - Duration::seconds(60));
    let stats = group.decompress().unwrap();
    assert_eq!(stats[0].collected_at, end - Duration::seconds(120));
    assert_eq!(stats[1].collected_at, end - Duration::seconds(60));
    ...
    for e in 0..3 {
        ...
        let mut stats = Vec::new();
        for _ in 1..=10 {
            stat.collected_at += Duration::seconds(60);
            stats.push(stat.clone());
        }
        CompressedQueryStats::store(db, stats).await.unwrap();
    }
    let start = start + Duration::seconds(3 * 60); // minute 3, skipping the first 2 minutes in the group
    let end = start + Duration::seconds(23 * 60); // minute 26, skipping the last 4 minutes in the group
    let groups = CompressedQueryStats::load(db, &[database_id], start, end).await.unwrap();
    assert_eq!(3, groups.len());
    let (mut calls, mut min, mut max) = (0, chrono::Utc::now(), DateTime::UNIX_EPOCH);
    for group in groups {
        for stat in group.decompress().unwrap() {
            calls += stat.calls;
    ...
    DB_POOL.manager().statement_caches.clear();
    {
        ...
        let end = end + Duration::seconds(5 * 60); // minute 31
        ...
        let (mut calls, mut new_col, mut min, mut max) = (0, 0, chrono::Utc::now(), DateTime::UNIX_EPOCH);
        for group in groups {
            for stat in group.decompress().unwrap() {
                calls += stat.calls;
...
async fn aggregate() {
    ...
    let start: chrono::DateTime<Utc> = DateTime::<Utc>::from(end - Duration::seconds(3600)).duration_trunc(chrono::Duration::hours(1)).unwrap().into();
    let end = start + Duration::seconds(3600);
