...
pub async fn store() -> Result<()> {
    ...
    let sql = "
        DROP TABLE IF EXISTS comparison_pco;
        CREATE TABLE comparison_pco (
        ...
        CREATE INDEX ON comparison_pco USING btree (database_id);
        CREATE INDEX ON comparison_pco USING btree (end_at, start_at);
    ";
    ...
    for database_id in database_ids {
        ...
        for rows_ in grouped_rows.into_values() {
            ...
            let total_time: Vec<i64> = total_time.into_iter().map(|t| (t * 100.0).round() as i64).collect();
            let io_time: Vec<i64> = io_time.into_iter().map(|t| (t * 100.0).round() as i64).collect();
...
pub async fn load() -> Result<()> {
    ...
    impl CompressedQueryStats {
        ...
        pub fn decompress(self) -> Vec<QueryStat> {
            ...
            let total_time: Vec<i64> = ::pco::standalone::simple_decompress(&self.total_time).unwrap();
            let io_time: Vec<i64> = ::pco::standalone::simple_decompress(&self.io_time).unwrap();
            ...
            for (index, collected_at) in collected_at.into_iter().enumerate() {
                ...
                results.push(QueryStat {
                    ...
                    calls: calls[index],
                    rows: rows[index],
                    total_time: total_time[index] as f64 / 100.0,
                    io_time: io_time[index] as f64 / 100.0,
