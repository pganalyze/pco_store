...
pub async fn store(mult: u8) -> Result<()> {
    let db = &mut DB_POOL.get().await.unwrap();
    let sql = &format!(
        "
        DROP TABLE IF EXISTS float_mult_{mult};
        CREATE TABLE float_mult_{mult} (
        ...
        CREATE INDEX ON float_mult_{mult} USING btree (database_id);
        CREATE INDEX ON float_mult_{mult} USING btree (end_at, start_at);
    "
    );
    ...
    for database_id in database_ids {
        ...
        for rows_ in grouped_rows.into_values() {
            ...
            let total_time: Vec<i64> = total_time.into_iter().map(|t| (mult as f64 * t).round() as i64).collect();
            let io_time: Vec<i64> = io_time.into_iter().map(|t| (mult as f64 * t).round() as i64).collect();
