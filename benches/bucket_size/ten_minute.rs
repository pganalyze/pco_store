...
pub async fn store() -> Result<()> {
    ...
    let sql = "
        DROP TABLE IF EXISTS bucket_size_ten_minute;
        CREATE TABLE bucket_size_ten_minute (
        ...
        CREATE INDEX ON bucket_size_ten_minute USING btree (database_id);
        CREATE INDEX ON bucket_size_ten_minute USING btree (end_at, start_at);
    ";
    ...
    for database_id in database_ids {
        let sql = "
            SELECT collected_at, collected_secs, fingerprint, postgres_role_id, calls, rows, total_time, io_time, shared_blks_hit, shared_blks_read
            FROM query_stats WHERE database_id = $1
        ";
        let rows = db.query(sql, &[&database_id]).await?;
        let sql = "
            COPY bucket_size_ten_minute (
        ...
        for row in rows {
            let collected_at: Vec<SystemTime> = row.get(0);
            let collected_secs: Vec<i64> = row.get(1);
            let fingerprint: Vec<i64> = row.get(2);
            let postgres_role_id: Vec<i64> = row.get(3);
            let calls: Vec<i64> = row.get(4);
            let rows: Vec<i64> = row.get(5);
            let total_time: Vec<f64> = row.get(6);
            let io_time: Vec<f64> = row.get(7);
            let shared_blks_hit: Vec<i64> = row.get(8);
            let shared_blks_read: Vec<i64> = row.get(9);
