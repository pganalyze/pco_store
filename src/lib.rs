...
pub fn store(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for field in model.fields.iter() {
        ...
        load_fields.push(quote! { #ident: if fields.#ident { row.get(#index) } else { Default::default() }, });
        index += 1;
    }
    ...
    quote! {
        ...
        impl #packed_name {
            ...
            pub async fn load(db: &deadpool_postgres::Object, filter: Filter, fields: impl TryInto<Fields>) -> anyhow::Result<Vec<#packed_name>>
            {
                let Ok(fields) = fields.try_into() else {
                    // TODO: can't use ? operator here for some reason
                    // return Err(anyhow::Error::msg("unknown field".to_string()));
                    return Ok(Vec::new());
                };
                // let fields: Fields = fields.try_into().unwrap_or_default();
            ...
            pub async fn delete(db: &deadpool_postgres::Object, filter: Filter, fields: impl TryInto<Fields>) -> anyhow::Result<Vec<#packed_name>> {
                let Ok(fields) = fields.try_into() else {
                    // TODO: can't use ? operator here for some reason
                    // return Err(anyhow::Error::msg("unknown field".to_string()));
                    return Ok(Vec::new());
                };
                // let fields = fields.try_into().unwrap();
                // let fields = Fields::default();
                #load_checks
                let sql = #delete_sql;
                let mut results = Vec::new();
...
fn filter(model: ItemStruct, args: Arguments, using_chrono: bool, timestamp_ty: &Option<proc_macro2::TokenStream>) -> proc_macro2::TokenStream {
    ...
    let timestamp_helpers = timestamp.map(|timestamp| {
        ...
        quote! {
            ...
            pub fn bounds(&self) -> anyhow::Result<(#timestamp_ty, #timestamp_ty)> {
                use anyhow::Context;
            ...
            pub fn shift(&mut self, duration: #duration_type) -> anyhow::Result<()> {
                use std::ops::Add;
                let (start, end) = self.bounds()?;
    ...
    quote! {
        ...
        impl TryFrom<&[&str]> for Fields {
            ...
            fn try_from(input: &[&str]) -> Result<Self, Self::Error> {
                ...
                for s in input {
                    match *s {
                        #from
                        // f => return Err(anyhow::Error::msg("unknown field ".to_string() + f)),
                        f => return Err("unknown field"),
                    }
                }
                Ok(fields)
            }
        ...
        impl<'de> serde::de::Visitor<'de> for FieldsVisitor {
                ...
                A: serde::de::SeqAccess<'de>,
            {
                ...
                Fields::try_from(strings).map_err(serde::de::Error::custom)
            }
        }
    }
}
